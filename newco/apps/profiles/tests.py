...
class ProfilesCreateTestCase(TestCase):
    def test_profile_create(self):
        client1 = Client.objects.create(name='test user1',
...
class ProfilesListTestCase(TestCase):
    def setUp(self):
        client1 = Client.objects.create(name='test user1',
                                        email='test@example.com',
                                        address='example address')
        client2 = Client.objects.create(name='test user2',
                                        email='test2@example.com',
                                        address='example address')
        UserProfile.objects.create(profile_user=client1, info='True',
                                   status='single')
        UserProfile.objects.create(profile_user=client2, info='True',
                                   status='married')
